...
void main1() {
    ...
    // 遍历v1集合
    // 函数对象
    //for_each函数执行过程中，自动将容器中的元素传入Func的operator()函数中
    for_each(v1.begin(), v1.end(), Func<int>());
}

// 谓词 （根据参数个数还可以分一元谓词、二元谓词）
...
// 二元谓词
// 需求 ： 两个Vector(v1,v2),将v1和v2中所有的元素相加，然后重新装入一个新的容器v3中
template<typename T>
class MyAdd {
public:
...
void main3() {
    ...
    transform(v1.begin(), v1.end(), v2.begin(), v3.begin(), MyPrint<int>());
    ...
    //for_each的返回值是你传入的类型
    MyPrint<int> pobj1 = for_each(v3.begin(), v3.end(), MyPrint<int>());
}
...
void main5() {
    ...
    plus<string> myPlus1;
    string string1 = "alan1";
    string string2 = "alan2";
    myPlus1(string1, string2);

//    transform(v1.begin(), v1.end(), v2.begin(), v2.end(), plus<int>());
...
void main6() {
    ...
    //bind to second
    int num1 = count_if(v1.begin(), v1.end(), bind2nd(greater<int>(), 10));
    cout << "大于20的元素的个数：" << endl;
